...
Voici une structure pour démarrer, pensez à utiliser les composants material plutôt que du html pur:
...
Nous allons toutefois privilégier l'utilisation de `functional components`, et utiliser le hook `useParams();` vous pouvez récupérer la valeur de la route. [Suivez cet exemple](https://reacttraining.com/react-router/web/example/url-params).

Une fois la valeur de la route récupérée, modifier l'état de l'application, pour qu'il corresponde au mur à afficher. Puis implémenter une route et une n'affichant qu'un post-it.
